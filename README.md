...
bashgit clone https://github.com/YOUR_USERNAME/payment-reconciliation-api.git
...
envDB_USER=your_mysql_username
...
2. Create an Order
bashcurl -X POST "http://localhost:8000/orders" \
  -H "Content-Type: application/json" \
  -d '{
    "merchant_id": "MERCHANT_A",
...
3. Create a Payment
bashcurl -X POST "http://localhost:8000/payments" \
  -H "Content-Type: application/json" \
  -d '{
    "merchant_id": "MERCHANT_A",
    "merchant_order_id": "ORDER_001",
...
bashcurl -X POST "http://localhost:8000/reconciliation"
🔍 Reconciliation Algorithm
The system matches payments to orders using these rules:

Same merchant_order_id - Links payment to specific order
Same merchant_id - Both belong to same merchant
Exact amount match - Payment amount equals order amount
Date tolerance - Payment date within ±3 days of order date
Status validation - Both must have status='completed'
